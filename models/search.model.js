...
module.exports = {
    ...
    searchByNamePlus: (tukhoa,idCat,offset,sort) => db.load(`select * from sanpham sp join dmsp dm ON dm.id_SP = sp.id where ISNULL(sp.nguoiThang) AND dm.id_DM = ${idCat} AND sp.timeEnd - now() > 0 AND
    MATCH(sp.ten_SP, sp.moTaSP) AGAINST('${tukhoa}' IN NATURAL LANGUAGE MODE) ORDER BY ${sort} limit ${config.paginate.limit} offset ${offset}`),
};
